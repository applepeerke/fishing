...
from src.domains.login.api import login_register, password, password_validate, login_initialize, \
    login_password_reset, login_login, \
...
# Login
app.include_router(login_register, prefix=f'{prefix}/login/register', tags=['Login - Send otp'])
app.include_router(login_initialize, prefix=f'{prefix}/login/initialize', tags=['Login - Receive otp'])
