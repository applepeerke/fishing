...
async def create_random_fishing_data(
    ...
    await populate_fishing_with_random_data(
        db, int(no_of_fishingwaters), int(no_of_fishes), int(no_of_catches))
...
async def populate_fishing_with_random_data(db, no_of_fishingwaters, no_of_fishes, no_of_fishermen, no_of_catches=0):
    ...
    all_fishermen = await _create_fishermen(db, fake_fishermen, no_of_fishermen)
...
async def _create_fishermen(db, items: list, no_of_fishermen: int) -> [Fisherman]:
    no_of_fishermen = min(len(items), no_of_fishermen)
    index_set = _get_random_index_set(items, no_of_fishermen)
    items = [items[i] for i in range(len(items)) if i in index_set]
...
async def _create_random_fishing_relations(db, all_fishingwaters, all_fishermen, all_fishes):
    ...
    mean_density = sum(f.density for f in all_fishingwaters) / len(all_fishingwaters)
